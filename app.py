...
if os.path.isfile(csv_file_shift):
    ...
    df_shift = pd.DataFrame(columns=["Load Name", "Starting Time", "Power Consumption"])

# Check if the CSV file exists for sheddable loads
if os.path.isfile(csv_file_shed):
    df_shed = pd.read_csv(csv_file_shed)
else:
    df_shed = pd.DataFrame(columns=["Load Name", "Power Consumption"])
...
def app_layout():
    ...
    with st.sidebar.form("Shiftable Load Form"):
        ...
        starting_time_shift = st.sidebar.number_input('Enter the Starting Time', format="%g", key="shift_start_time")
        load_power_shift = st.sidebar.number_input('Enter the Power Consumption', format="%g", key="shift_power_consumption")
    ...
    with st.sidebar.form("Sheddable Load Form"):
        ...
        load_power_shed = st.sidebar.number_input('Enter the Power Consumption', format="%g", key="shed_power_consumption")

        add_shed = st.form_submit_button('Add to Sheddable')
        delete_shed = st.form_submit_button('Delete Data from Sheddable')

    if add_shift:
        df_shift.loc[len(df_shift)] = [load_name_shift, starting_time_shift, load_power_shift]
    ...
    if add_shed:
        df_shed.loc[len(df_shed)] = [load_name_shed, load_power_shed]
    ...
    if st.button("Run Optimization"):
        ...
        battery_schedule_opt = run_optimization(T, battery_state_0, solar_forecasting, building_forecasting,
                                               actual_solar, actual_building, electricity_tariff,
                                               shiftable_loads, sheddable_loads)
